...
namespace OodlesParty
{
    public class AnalogStick : UniversalButton
    {
        ...
        public override void OnPointerDown(PointerEventData eventData)
        {
            if (state == ButtonState.Active)
            {
                ...
                tempFloat = (fingerPosition - dpadInner.position).magnitude;

                if (tempFloat < innerRadius)
                {
                ...
                {
                    tempVec = dpadInner.position - fingerPosition;
                    tempVec = Vector3.ClampMagnitude(tempVec, aimerRadius);
                    tempVec = fingerPosition + tempVec;

                    aimer.position = new Vector3(
                        tempVec.x,
                        tempVec.y < aimerRadius ? aimerRadius : tempVec.y,
                        tempVec.z
                    );
                }
    ...
    public class AnalogStickInspector : UnityEditor.Editor
    {
